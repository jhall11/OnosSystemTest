...
class FakeController(Thread):
    ...
    def __init__(self, name, port, timeout=DefTimeout):
        ...
        self.listen_sock.bind(('10.128.100.55',port))
    ...
    def run(self):
        print "Inside run function of FakeController class" if debug == True else ''
	ctl_logger.info(self.name + ": Starting io loop")
        while self.active :
	    #print "Calling try block ****"
            try:
		print "Inside try block and waiting to accept connection"
    ...
    def __init__(self,name, sock=None,host=None,port=None):
        ...
        if sock != None :
            ...
            try:
                ...
                raise FvExcept("connection refused")
            if self.sock == None :
